...
function authorize(req, res) {
  ...
  if (redirect_uri === undefined) return res.status(400).send("bad request");
  db.execute(
      ...
      if (err) throw err;
        ...
        if (results.length == 1) {
          ...
          if (user.password === password) {
            ...
            let authCode = new Buffer(JSON.stringify(authCodeObj)).toString(
              "base64"
            );
            return res.redirect(
              redirect_uri + "&authorization_code=" + authCode
            );
          }
        }
